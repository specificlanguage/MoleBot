...
def change_mole(server_id: int):
    ...
    set_mole_command = """INSERT INTO settings(discord_id, mole) VALUES(%s, %s) ON CONFLICT (discord_id) DO
                            UPDATE SET mole = %s;"""
    with conn.cursor() as curs:
        ...
        curs.execute(set_mole_command, [server_id, new_setting, new_setting])
        conn.commit()
        return new_setting
...
def get_mole(server_id: int):
    ...
    with conn.cursor() as curs:
        ...
        if len(rows) == 0:  # The discord doesn't exist in the db yet because it's disabled by default.
            ...
            set_mole_command = """INSERT INTO settings(discord_id, mole) VALUES(%s, %s) ON CONFLICT (discord_id) DO
                            UPDATE SET wiki = %s;"""
            curs.execute(set_mole_command, [server_id, False, False])
            conn.commit()
            return "New server"
...
def set_wiki_setting(server_id: int, setting: bool):
    ...
    with conn.cursor() as curs:
        ...
        set_mole_command = """INSERT INTO settings(discord_id, wiki) VALUES(%s, %s) ON CONFLICT (discord_id) DO
                                UPDATE SET wiki = %s;"""
        curs.execute(set_mole_command, [server_id, setting, setting])
        conn.commit()
